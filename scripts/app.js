...
const page = {
	...
	popup: {
		...
		iconField: document.querySelector('.popap__form input[name="icon"]'),
	}
};
...
function rerenderMenu(activeHabbit) {
    for (const habbit of habbits) {
        ...
        if (!existed) {
            ...
            element.innerHTML = `<img src="images/logo/${habbit.icon}.svg " alt="${habbit.icon}"/>`;
...
function rerenderContent(activeHabbit) {
    ...
    for (const index in activeHabbit.days) {
		...
		<button class="habbit__delete" onclick="deleteDay(${index})">
			<img src="images/logo/delete.svg" alt="Удалить день ${index + 1}"/>
		</button>`;
